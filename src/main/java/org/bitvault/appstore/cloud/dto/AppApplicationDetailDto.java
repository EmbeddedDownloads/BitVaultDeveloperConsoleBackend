...
public class AppApplicationDetailDto {

	private Integer appDetailId;

	private org.bitvault.appstore.cloud.dto.AppApplication application;

	public org.bitvault.appstore.cloud.dto.AppApplication getApplication() {
		return application;
	}

	public void setApplication(org.bitvault.appstore.cloud.dto.AppApplication application) {
	...
	public static AppApplicationDetailDto populateAppApplicationDetailDto(AppDetail appDetail, AppApplication app) {
		AppApplicationDetailDto appApplicationDetailDto = new AppApplicationDetailDto();
		try {
			BeanUtils.copyProperties(appDetail, appApplicationDetailDto);
			appApplicationDetailDto.setApplication(app);
		} catch (Exception e) {
			e.printStackTrace();
			throw new BitVaultException(ErrorMessageConstant.SOME_ERROR_OCCURED);
		}
		return appApplicationDetailDto;
	}
}
